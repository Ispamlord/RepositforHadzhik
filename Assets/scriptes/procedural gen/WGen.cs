...
public class WGen : MonoBehaviour
{
    ...
    void InitializeGrid()
    {
        ...
        Walkers = new List<WObj>();

        Vector3Int TileCenter = new Vector3Int(gridHandler.GetLength(0) / 2, gridHandler.GetLength(1) / 2, 0);

        WObj curWalker = new WObj(new Vector2(TileCenter.x, TileCenter.y), GetDirection(), 0.5f);
    ...
    IEnumerator CreateFloors()
    {
        while ((float)TileCount / (float)gridHandler.Length < FillPercentage)
        {
            ...
            foreach (WObj curWalker in Walkers)
            {
    ...
    void ChanceToCreate()
    {
        ...
        for (int i = 0; i < updatedCount; i++)
        {
            if (UnityEngine.Random.value < Walkers[i].ChanceToChange && Walkers.Count < MaximumWalkers)
            {
                ...
                WObj newWalker = new WObj(newPosition, newDirection, 0.5f);
